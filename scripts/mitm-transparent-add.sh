...
if [ "$#" -eq 2 ]; then
    sudo iptables -t nat -A OUTPUT -p tcp -d $2 --dport $1 -j DNAT --to-destination 127.0.0.1:8080
else
    sudo iptables -t nat -A OUTPUT -p tcp --dport $1 -j DNAT --to-destination 127.0.0.1:8080
fi